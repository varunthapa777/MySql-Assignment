...
ALTER TABLE dept
ADD COLUMN hod VARCHAR(15),
ADD COLUMN strength VARCHAR(15);
...
ALTER TABLE dept
ADD
    CONSTRAINT chk_deptno CHECK(
        deptno IN ('D001', 'D002', 'D003')
    );
...
CREATE INDEX idx_client_no ON client_mast(client_no);

SHOW INDEXES FROM client_mast;
...
CREATE View clientView AS SELECT * FROM client_mast;
